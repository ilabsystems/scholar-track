...
<div class="py-12">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <!-- Back Button -->
        <div class="mb-8">
            <a href="{{ route('scholarships.index') }}" class="inline-flex items-center text-blue-600 hover:text-blue-700 font-medium">
            ...
            <div class="bg-gradient-to-r from-blue-900 to-blue-800 px-8 py-12">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <form action="{{ route('scholarships.favorite', $scholarship) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="shrink-0 transition hover:scale-110 p-3 bg-white/20 rounded-full hover:bg-white/30" title="{{ $isFavorited ? 'Remove from favorites' : 'Add to favorites' }}">
                            @if($isFavorited)
                                <i class="fas fa-heart text-red-300 text-2xl"></i>
                            @else
        ...
        <div class="mt-12">
            <h2 class="text-2xl font-bold text-gray-900 mb-6">Other Scholarships You Might Like</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <!-- Placeholder for related scholarships -->
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center text-gray-500">
                    <p>More scholarships coming soon</p>
                </div>
            </div>
        </div>
    </div>
